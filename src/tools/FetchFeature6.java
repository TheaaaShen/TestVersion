...
public class FetchFeature6 {
	double P = 0.75;
	double P_Noise=0.1;
	public ArrayList<PeakEntropyInfo> FetchMatchMzPeak2(
			ArrayList<CompareInfo> candiCompareInfoList,int cutTime,int peakLevel) {
		/*
		 * preProbScore init
		 */
		...
		Hashtable<DataPoint, ArrayList<Integer>> peakIDHash = new Hashtable<DataPoint, ArrayList<Integer>>();
		 ...
		 * glycan structures
		 * j can be see an the StrucID
		 */
		ArrayList<DataPoint> subCandiPeakList = new ArrayList<DataPoint>();
		/*
		 * each union matched exp peak
		 */
		int topIntensNums=5;
		
		DataPoint[] experSp=this.getUnionMatchedPeakArray(candiCompareInfoList,topIntensNums);
		
		for (int i = 0; i < experSp.length; i++) {
			DataPoint iterExperiPeak = experSp[i];
			...
			/*
			 * common_matched_peak search
			 * more than one candi_struc matched this exp_peak
			 */
			if (Detecter > 1) {
		...
		/*
		 * count entropy
		 * @subCandiiPeakList contains all candidate peaks for next stage spectrum
		 */
		double eEntropy=0;
		ArrayList<PeakEntropyInfo> peakEntropyInfoList=new ArrayList<PeakEntropyInfo>(); 
		for (DataPoint iterCommPeak : subCandiPeakList) {
			ArrayList<Integer> candiIDList=peakIDHash.get(iterCommPeak);
			/*
			 * for each candidate peak count the average entropy
			 */
			PeakEntropyInfo peakEntropyInfo=this.countProbScore(iterCommPeak,candiIDList,
					candiCompareInfoList ,preProbArray,cutTime);
			
			/*
			eEntropy=this.countExpectEntropy(probScoreArrayList);
			double peakMz=FormatNum.DoubleFormat(iterCommPeak.getMZ(),3);
			double peakIntens=FormatNum.DoubleFormat(iterCommPeak.getIntensity(),3);
			double peakEntropy=FormatNum.DoubleFormat(eEntropy,3);
			System.out.println(peakMz+"\t"+
			peakIntens+"\t"+
			candiIDList+"\t"+peakEntropy);
			PeakEntropy peakEntropyInfo=new PeakEntropy(peakMz,peakIntens,candiIDList,peakEntropy);
			*/
	...
	public DataPoint[] getUnionMatchedPeakArray(ArrayList<CompareInfo> candiCompareInfoList,int topIntensNums)
	{
		...
		for(int i=0;i<candiCompareInfoList.size();i++)
		{
			...
			if(tmpCandiInfo!=null)
			{
				...
				{
					DataPoint tmpPeak=new DataPoint(tmpPeakInfoList.get(j).getPeakMz(),tmpPeakInfoList.get(j).getPeakIntens());
		...
		Collections.sort(peakList,new DataPointIntensComparator());
		List <DataPoint> topIntensPeakList;
		...
		Collections.sort(topIntensPeakList,new DataPointMzComparator());
		
		DataPoint[] reArray=new DataPoint[intensTop];
	...
	/*
	 * input: a experiment peak, candidate structure,  pre_probability, structure cut time
	 * output: all possible spectrum correspond candidate structure probability score 
	 */
	public PeakEntropyInfo countProbScore(DataPoint comPeak,ArrayList<Integer> candiIDList,ArrayList<CompareInfo> candiCompareInfoList,double[] preProbArray,int cutTime)
	{
		...
		for (int i = 0; i < candiSubPeakList.size(); i++) {
			 ...
			 */
			for (ArrayList<Double> iterList : this
					.countCombination(featureList)) {
				...
				probArray[i] = Math.pow(P, matchNum);//* Math.pow((1 - P), unionNum-matchNum);
//						* Math.pow((1 - P), unionNum-matchNum);
				...
				for (int j = 0; j < candiSubPeakList.size(); j++) {
					if (i != j) {
						...
						matchNum=shareSame;
						probArray[j] = Math.pow(P, matchNum)//* Math.pow((1 - P), unionNum-matchNum);
								* Math.pow((1 - P), unionNum-matchNum);
...
//		return this.arrayNormalize(probArrayList, candiIDList, preProbArray);
	...
	public ArrayList<Double> getSubSpMzList(CompareInfo compareInfo,DataPoint comPeak,int cutTime)
	{
	...
	private ArrayList<ArrayList<Double>> countCombination(
