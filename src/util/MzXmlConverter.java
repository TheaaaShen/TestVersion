	...
	public void writeOut(String result_file) {
		...
		try {
			BufferedWriter outfile = new BufferedWriter(new FileWriter(
					result_file));
			for (DataPoint iter : peak_group) {
				outfile.write(df2.format(iter.mz) + "\t" + df1.format(iter.Intensity));
				outfile.newLine();
			}
			outfile.flush();
			outfile.close();
