...
public class MzXMLReader {
	...
	public  DataPoint[] get_peak_list() {
		 DataPoint[] peak_group = decode_str(this.peak_str,
	...
	public ArrayList<double[]> getPeakList(){
		...
		for(DataPoint iterPeak: peak_group)
		{
			double[] peakPair=new double[2];
			peakPair[0]=iterPeak.mz;
			peakPair[1]=iterPeak.Intensity;
			peakList.add(peakPair);
		}
		return peakList;
	}

	public DataPoint[] getPeakData() {
		DataPoint[] peak_group = decode_str(this.peak_str,
				Integer.parseInt(this.peak_num));
		return peak_group;
	}
	...
	public double[][] getPeakGroup() {
		DataPoint[] peak_data = decode_str(this.peak_str,
				Integer.parseInt(this.peak_num));
	...
	public DataPoint[] decode_str(String peak_str, int peak_count) {
		if (peak_str == null)
			return null;
