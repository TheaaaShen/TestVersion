...
public class DataFilter {
	public static DataPoint[] experiSpPeakFiltMaxSumIntens(DataPoint[] expIonArray,
			double filtIntensRatio) {
		DataPoint[] filtedIonArray;
		ArrayList<DataPoint> dataPointList = new ArrayList<DataPoint>();
		ArrayList<DataPoint> filtedPointList = new ArrayList<DataPoint>();
		double sumIntens = 0;
		double filtSumIntens = 0;
		double filtRatio = filtIntensRatio;
		int index;
		for (DataPoint iterPeak : expIonArray) {
			sumIntens = sumIntens + iterPeak.getIntensity();
			dataPointList.add(iterPeak);
		}
		Collections.sort(dataPointList, new DataPointIntensComparator());

		for (index = 0; index < dataPointList.size(); index++) {

			filtSumIntens = filtSumIntens
					+ dataPointList.get(index).getIntensity();
		...
		Collections.sort(filtedPointList, new DataPointMzComparator());

		filtedIonArray = new DataPoint[filtedPointList.size()];
	...
	public static DataPoint[] experiSpPeakFiltMinSinglePeak(DataPoint[] expIonArray,
			double filtRelateRatio) {
		DataPoint[] filtedIonArray;
		ArrayList<DataPoint> dataPointList = new ArrayList<DataPoint>();
		ArrayList<DataPoint> filtedPointList = new ArrayList<DataPoint>();
		double sumIntens = 0;
	...
	public static DataPoint[] experiSpPeakFiltMinSinglePeak(ArrayList<DataPoint> expIonArray,
			double filtRelateRatio) {
		DataPoint[] filtedIonArray;
		ArrayList<DataPoint> dataPointList =expIonArray;
		ArrayList<DataPoint> filtedPointList = new ArrayList<DataPoint>();
		double sumIntens = 0;
		double filtRelateIntens = 0;
		double filtRatio = filtRelateRatio;
	...
	public static DataPoint[] experiSpPeakFiltMinSinglePeakIntens(DataPoint[] expIonArray,
			double minIntens) {
		DataPoint[] filtedIonArray;
		ArrayList<DataPoint> filtedPointList = new ArrayList<DataPoint>();		
	...
	public static DataPoint[] experiSpPeakFiltIntensTopRank(DataPoint[] expIonArray,
			int topM) {
		DataPoint[] filtedIonArray;
		ArrayList<DataPoint> dataPointList = new ArrayList<DataPoint>();
		ArrayList<DataPoint> filtedPointList = new ArrayList<DataPoint>();
	...
	public static DataPoint[] getNormalizedPeakArray(DataPoint[] expIonArray,double maxIntens)
	{
	...
	/* 
	 * filter the NodeList 
	 * delete the cut more 2 times node not have the cut from "right end NGA"
	 */
	...
	public static ArrayList<FragNode> filterTheorySpSameMassIon(ArrayList<FragNode> nodeList) {
		...
		for (int i = 0; i < nodeList.size();) {
			...
			while (i < nodeList.size()
					&& (int) tmpNode.getSubtreeMass() == (int) nodeList.get(i)
							.getSubtreeMass()
					&& tmpNode.getStrucID()
							.equals(nodeList.get(i).getStrucID())) {
				FragNode detectedNode = nodeList.get(i);
				if (tmpNode.getIonTypeNote().length() > detectedNode
						.getIonTypeNote().length()) {
					tmpNode = detectedNode;
					candiNodeAsciiList.clear();
					candiNodeList.clear();

				} else if (tmpNode.getIonTypeNote().length() == detectedNode
						.getIonTypeNote().length()) {
					int detectedAsciiValue = countStrAsciiValue(detectedNode
							.getIonTypeNote());
	...
	public static DataPoint[] filterIsoIons(DataPoint[] expSp,double WIN)
	{
		...
		DataPoint[] reArray=new DataPoint[peakList.size()];
